    ...
    <aside className="flex flex-col gap-[24px] items-start w-full">
      ...
      <div className="flex flex-col gap-[32px] items-start w-full">
        <div className="flex items-center justify-between w-full">
          <h2 className="text-[40px] font-semibold leading-normal text-grey-950 whitespace-nowrap">
            Filters
          </h2>
          {activeCount > 0 && (
            <button
            ...
            >
              ...
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none" className="shrink-0">
                <path d="M1 1l8 8M9 1l-8 8" stroke="#8A8A8A" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
            </button>
          )}
        </div>
        ...
        <div className="flex flex-col gap-[56px] items-start w-full">
          ...
          <section className="flex flex-col gap-[24px] items-start w-full">
            <h3 className="text-[18px] font-medium leading-normal text-grey-500 w-full">
              Categories
            </h3>
              ...
              {categories.map(cat => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-[10px] h-[39px] px-[12px] py-[8px] rounded-[12px] border-0 cursor-pointer transition-colors ${
                      active
                        ? 'bg-primary-100 text-primary'
                        : 'bg-white text-grey-500 hover:bg-grey-100'
                    }`}
                  >
                    {Icon && <Icon className="size-[24px] shrink-0" />}
              ...
              {topics.map(topic => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-center h-[39px] px-[12px] py-[8px] rounded-[12px] border-0 cursor-pointer transition-colors ${
                      active
                        ? 'bg-primary-100 text-primary'
                        : 'bg-white text-grey-500 hover:bg-grey-100'
                    }`}
          ...
          <section className="flex flex-col gap-[24px] items-start">
            <h3 className="text-[18px] font-medium leading-normal text-grey-500">
              Instructor
            </h3>
              ...
              {instructors.map(inst => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-[12px] w-full px-[12px] py-[8px] rounded-[12px] border-0 cursor-pointer transition-colors ${
                      active
                        ? 'bg-primary-100 text-primary'
                        : 'bg-white text-grey-500 hover:bg-grey-100'
                    }`}
      ...
      <div className="border-t border-grey-300 pt-[16px] w-full">
        <p className="text-[14px] font-medium text-grey-400 leading-normal">
